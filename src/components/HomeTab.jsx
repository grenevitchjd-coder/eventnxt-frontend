// eventnxt-frontend: src/components/HomeTab.jsx
//
// "Event page" tab — the public-page editor for the shell's CURRENT event.
// (Formerly also held the org event list; the Dashboard sidebar owns event
// selection now, and + New Event lives there too.)
...
export default function HomeTab({ onToast, event }) {
  ...
  // The Dashboard shell owns event selection and remounts this tab
  // (key={eventId}) when it changes — load the current event's profile
  // once on mount.
  useEffect(() => {
    api
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true)
    try {
      const saved = await api.saveEventProfile(event.id, buildFullPayload())
      const isFirstSave = !profile
      setProfile(saved)
      setForm({ ...form, slug: saved.slug })
      onToast('Saved')
      if (isFirstSave) loadProfileExtras(event.id)
    } catch (err) {
  ...
  const handleBannerChange = async (e) => {
    ...
    try {
      const updated = await api.uploadBannerPhoto(event.id, file)
      setProfile(updated)
  ...
  const handleLogoChange = async (e) => {
    ...
    try {
      const updated = await api.uploadLogo(event.id, file)
      setProfile(updated)
  ...
  const togglePublish = async () => {
    ...
    try {
      const updated = profile.is_published
        ? await api.unpublishEventProfile(event.id)
        : await api.publishEventProfile(event.id)
      setProfile(updated)
  ...
  const handleAddContact = async (e) => {
    ...
    try {
      const created = await api.createProfileLink(event.id, { kind: 'contact', ...contactForm })
  ...
  const handleAddSocial = async (e) => {
    ...
    try {
      const created = await api.createProfileLink(event.id, { kind: 'social', ...socialForm })
      setLinks([...links, created])
  ...
  const handleDeleteLink = async (link) => {
    try {
      await api.deleteProfileLink(event.id, link.id)
  ...
  const handleAddScheduleItem = async (e) => {
    ...
    try {
      const created = await api.createScheduleItem(event.id, {
  ...
  const handleGalleryPhotoChange = async (e) => {
    ...
    try {
      const created = await api.uploadGalleryPhoto(event.id, file)
  ...
  const handleDeleteGalleryPhoto = async (photo) => {
    try {
      await api.deleteGalleryPhoto(event.id, photo.id)
